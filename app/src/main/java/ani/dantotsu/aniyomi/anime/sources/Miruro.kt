...
class Miruro(
    ...
    override var baseUrl: String
        get() = preferences.getString(PREF_MIRROR_KEY, PREF_MIRROR_DEFAULT) ?: PREF_MIRROR_DEFAULT
        set(value) {
            preferences.edit().putString(PREF_MIRROR_KEY, value).apply()
        }

    private val markFillers: Boolean
        get() = preferences.getBoolean(PREF_MARK_FILLERS_KEY, PREF_MARK_FILLERS_DEFAULT)

    private val hideFillers: Boolean
        get() = preferences.getBoolean(PREF_HIDE_FILLERS_KEY, PREF_HIDE_FILLERS_DEFAULT)

    private val includeAllSubTypes: Boolean
        get() = preferences.getBoolean(PREF_INCLUDE_ALL_SUB_TYPES_KEY, PREF_INCLUDE_ALL_SUB_TYPES_DEFAULT)

    private val stripHtml: Boolean
        get() = preferences.getBoolean(PREF_STRIP_HTML_KEY, PREF_STRIP_HTML_DEFAULT)

    private val mergeAcrossProviders: Boolean
        get() = preferences.getBoolean(PREF_MERGE_PROVIDERS_KEY, PREF_MERGE_PROVIDERS_DEFAULT)

    private val preferredTitleStyle: String
        get() = preferences.getString(PREF_TITLE_STYLE_KEY, PREF_TITLE_STYLE_DEFAULT) ?: PREF_TITLE_STYLE_DEFAULT

    private val preferredProvider: String
        get() = preferences.getString(PREF_PROVIDER_KEY, PREF_PROVIDER_DEFAULT) ?: PREF_PROVIDER_DEFAULT

    private val preferredSubType: String
        get() = preferences.getString(PREF_SUB_TYPE_KEY, PREF_SUB_TYPE_DEFAULT) ?: PREF_SUB_TYPE_DEFAULT
    ...
    override fun searchAnimeParse(response: Response): AnimesPage =
        parseAnimeListResponse(response, fallbackKeys = listOf("results", "data"))
    ...
    override fun animeDetailsParse(response: Response): SAnime {
        ...
        val title = resolveTitle(titleObj, preferredTitleStyle)
    ...
    override fun videoListParse(response: Response): List<Video> {
        ...
        if (!includeAllSubTypes || subTypesObj == null || subTypesObj.length() <= 1) return videos
    ...
    override fun List<Video>.sort(): List<Video> {
        ...
        val subTypeLabel = when (preferredSubType) {
            "sub" -> "Sub"
            "dub" -> "Dub"
            "ssub" -> "Soft Sub"
            else -> preferredSubType.replaceFirstChar { it.uppercase() }
        }
    ...
    override fun setupPreferenceScreen(screen: PreferenceScreen) {
        ...
        screen.addPreference(
            SwitchPreference(context).apply {
                ...
                setOnPreferenceChangeListener { _, newValue ->
                    markFillersPref.isEnabled = !(newValue as Boolean)
                    true
                }
            }
        )
...
@Serializable
private class AnilistMalIdResponse(
    val data: DataObject,
) {
    @Serializable
    class DataObject(
...
@Serializable
private class JikanEpisodesDto(
